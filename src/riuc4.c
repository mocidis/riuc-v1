...
void set_snd_device(riuc_data_t *riuc_data) {
    ...
    for (i = 0; i < MAX_NODE; i ++) {
        riuc_data->config[i].streamer_idx = riuc_data->node[node_idx].streamer_dev_idx;
        riuc_data->config[i].receiver_idx = riuc_data->node[node_idx].receiver_dev_idx;
    }
}
        ...
        switch(option[0]) {
            ...
            case 'c':
                for (i = 0; i < MAX_NODE; i++) {
                    idx = atoi(option[i+1]);   
                    if (idx != i+1) {
                        streamer_config_dev_source(riuc_data.node[idx].streamer, config[i+1].streamer_idx);
                        receiver_config_dev_sink(riuc_data.node[idx].receiver, config[i+1].receiver_idx);
                    }
                    if (idx == -1) {
                        for (j = i; j < MAX_NODE; j++) {
                            streamer_config_dev_source(riuc_data.node[j+1].streamer, config[j].streamer_idx);
                            break;
                        }
                    }
                }
